...
- **The app shell is sized to `100dvh`, not `100vh`.** On mobile, `100vh` is the viewport with the browser's URL bar hidden — taller than what's actually visible — so a shell sized in `vh` used to need a drag to reach its own bottom edge. `100dvh` tracks the visible height instead (see `.app-shell` in `src/index.css`); the shell itself never scrolls, each view scrolls inside `<main>`.
- **Map and globe zoom are anchored, not fixed at the origin.** Scaling an SVG `<g>` or a WebGL camera naively drags whatever's at the transform's origin toward the center of the effect; `MapView`'s `zoomTo` re-solves the pan offset on every zoom so the point under the cursor, the pinch midpoint, or the viewport center — whichever triggered the zoom — is what stays put. The map's wheel-zoom listener is also attached by hand via `addEventListener('wheel', ..., { passive: false })` rather than React's `onWheel`, which React registers as passive and silently ignores `preventDefault()` on.
